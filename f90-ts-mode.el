...
;;++++++++++++++
;; additional offset: default case
;; if list of selected nodes to align with is empty, then suggest
;; continued line indentation

(defun f90-ts--align-continued-default-offsets (_list-context items-filtered _node-sym prev-stmt-1)
  "Return a list of default offsets depending on LIST-CONTEXT, NODE-SYM
and whether ITEMS-FILTERED has any nodes.
The default list consists of one single value which is standard
continued line indentation offset.
Offsets should be absolute column numbers."
  (unless items-filtered
    (list
     (+ (f90-ts--node-column prev-stmt-1) f90-ts-indent-continued)
     )))


(defun f90-ts--align-continued-tuple-offsets (list-context items-filtered node-sym prev-stmt-1)
  "Return a list of default offsets depending on context and whether
ITEMS-FILTERED has any nodes.
...
Offsets should be absolute column numbers."
  ...
  (cl-assert items-filtered
             nil "expected non-empty items-filtered")
...
determine relevant column positions and select column depending on CUR-COL.
The selected column is relative to column 0."
  (let* ((items-col-unsorted (seq-map
    ...
    ;;(f90-ts-log :indent "cont current column: %d" cur-col)
    (cond
     ((and items-col
...
Offset is computed relative to PREV-STMT."
  (seq-let (cur-col cur-line node-sym) (f90-ts--align-continued-location node)
    ...
    (let* ((get-items (f90-ts--get-list-context-prop :get-items list-context))
      ...
      (f90-ts-log :indent "cont offsets other: %s" offsets-other)

      (let ((offset-0 (f90-ts--align-continued-select
                       (append offsets-other items-filtered)
                       cur-col
                       variant)))
        ;; make the offset relative to first statement node
        ;; (important for indent-region and batch processing)
        (- offset-0 (f90-ts--node-column prev-stmt-1)))
      )))
...
;; see problems in continued lines with nested argument list in call statement
...
(defconst list-context-types
  '(("argument_list"        . (:get-items f90-ts--align-continued-arguments-items
                               :get-other f90-ts--align-continued-tuple-offsets))
    ...
    ("logical_expression"   . (:get-items f90-ts--align-continued-log-expr-items
                               :get-other f90-ts--align-continued-default-offsets))
